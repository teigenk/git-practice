...
#bring in the data for fig1
fig1 <- read_csv("fig1_ray.csv")

vars <- c("Year_Month", "Observed_SR", "ExpSR")
dat4 <- fig1[vars]
dat4 <- melt(dat4)

dat4$Year_Month <- factor(dat4$Year_Month, levels=c("1995/1", "1995/2", "1995/3", "1995/4", "1995/5", "1995/6", "1995/7", "1995/8", "1995/9", "1995/10", "1995/11", "1995/12",
...
dat4a <- dat4[which(dat4$variable == "Observed_SR"), ]
dat4b <- dat4[which(dat4$variable == "ExpSR"), ]
#dat4b <- dat4b %>% drop_na()


#Denoting those outside expected level
dat4 <- dat4 %>% mutate(outside=ifelse(value>1.5 | value<0.6222,1,0))
dat4a <- dat4a %>% mutate(outside=ifelse(value>1.5 | value<0.6222,1,0))
dat4b <- dat4b %>% mutate(outside=ifelse(value>1.5 | value<0.6222,1,0))

p<-ggplot(dat4a, aes(x = factor(Year_Month), y = value, color = factor(variable), shape = factor(outside))) +
  geom_point(size=2) +
  geom_line(data = dat4b, aes(y = value, x = factor(Year_Month),  group = 1) 
           ) +
  scale_x_discrete("Year/Month",
                   breaks = c(
                     ...
                     "2017/1",
                     "2018/1")) +
  scale_y_continuous("Extremely Preterm NH Black Twin Sex Ratio",
                     breaks=c(0.5, 0.7,0.9, 1.1,1.3,1.5,1.7,1.9)) + 
  scale_color_manual(
    "",
...
ggsave("fig1_ray2024.pdf", p, width = 9, height = 6, dpi = 300)
p


#bring in the data for fig2
fig2 <- read_csv("fig2_ray.csv")

vars <- c("Year_Month", "Diff", "Xdiff")
dat4 <- fig2[vars]
dat4 <- melt(dat4)
dat4$Year_Month <- factor(dat4$Year_Month, levels=c("1995/1", "1995/2", "1995/3", "1995/4", "1995/5", "1995/6", "1995/7", "1995/8", "1995/9", "1995/10", "1995/11", "1995/12",
                                                    "1996/1", "1996/2", "1996/3", "1996/4", "1996/5", "1996/6", "1996/7", "1996/8", "1996/9", "1996/10", "1996/11", "1996/12",
...
dat4a <- dat4[which(dat4$variable == "Diff"), ]
dat4b <- dat4[which(dat4$variable == "Xdiff"), ]

p2<-ggplot(dat4a, aes(x = Year_Month, y = value, color = factor(variable))) +
  geom_point(size=2) +
  geom_line(data = dat4b, aes(y = value, x = Year_Month,  group = 1)) +
  scale_x_discrete("Year/Month",
                   breaks = c(
                     "1995/1",
                     "1996/1",
                     ...
                     "2017/1",
                     "2018/1")) +
  scale_y_continuous("Birthweight Disparity in Grams"
                    ) + 
  scale_color_manual(
    "",
    values = c("#252525", "#252525"),
  ...
  theme_classic(base_size = 14)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=0.5)) +
  theme(legend.position="top") + theme_bw(base_size=14)
